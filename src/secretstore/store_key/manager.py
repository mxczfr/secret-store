...
class StoreKeyManager:
    ...
    def create_store_key(self, store_name: str, private_identity: PrivateIdentity, key: bytes):
        ...
        store_key = StoreKey(store_name, private_identity.fingerprint, aead_enc, ct_key_enc)

        # Because the store key is brand new, save it 
        self._dao.save(store_key)
    
    def get_store_encryption_key(self, store_name:str, private_identity: PrivateIdentity) -> bytes | None:
        ...
        recipient_context = self._get_hpke_cipher_suite().create_recipient_context(store_key.aead_enc, priv_hpke_key)
